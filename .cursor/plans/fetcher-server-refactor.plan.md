...
Phases 1–2 are complete: `@packages/http` exposes client, server-request, and static-build fetcher modes with flat `ApiError`; `@packages/nestjs-sdk` mutators and client wiring are documented. Kept as reference for xpertell-style `AuthSession` + provider patterns.

- Runtime modes: browser client, per-request server `createFetcher(settings, ctx)`, build/static
- `createBareFetcher` for refresh-only calls (no refresh recursion)
- Tests + `AGENTS.md` client/server/build matrix
...
Fivenines should support the same **client** wiring pattern already proven in production:
...
- `@packages/http` — duplicated Kubb fetch types + `baseFetch` in-package; React only in `@packages/http/react`.
...
### xpertell → fivenines mapping (documentation in AGENTS.md)

| xpertell | fivenines (after PR1/2) |
|----------|------------------------|
...
- Full `AuthSession` implementation until auth Kubb endpoints exist in fivenines (PR2 documents contract only if endpoints missing).
