...
overview: Reference for the Prisma Cursor plugin (skills, rules, hooks, Local/Remote MCP). Fivenines uses Prisma 7 in @apps/auth and @apps/nestjs; pass projectCWD to the app that owns the schema.
todos:
  - id: auth-remote
    content: Authenticate plugin-prisma-Prisma-Remote via MCP settings or mcp_auth when using Cloud features
    status: pending
  - id: add-prisma
    content: "Prisma in fivenines — apps/auth/src/prisma, apps/nestjs prisma schema"
    status: completed
isProject: false
---
...
Fivenines already uses Prisma 7 in `@apps/auth` (`apps/auth/src/prisma`, `fivenines_auth`) and `@apps/nestjs` (`apps/nestjs/prisma`, `fivenines`). Use Local MCP with `projectCWD` set to the app that owns the schema; hooks format/generate on schema edits.

## Remaining

- Authenticate **Prisma-Remote** only if you use Prisma Cloud (`mcp_auth` / MCP settings).
...
Agents pass **`projectCWD`** as the absolute path to your project root (e.g. `/Users/soheil/Workspace/fivenines` or a package subfolder once Prisma lives there).

### Prisma-Remote (authentication required)

- Hosted at `https://mcp.prisma.io/mcp`.
- **Status:** [`STATUS.md`](file:///Users/soheil/.cursor/projects/Users-soheil-Workspace-fivenines/mcps/plugin-prisma-Prisma-Remote/STATUS.md) reports the server **needs authentication**.
...
## Relevance to fivenines today

Prisma is **in use** in two workspaces:

| Workspace | Schema path | Database (typical) |
|-----------|-------------|-------------------|
| `@apps/auth` | `apps/auth/src/prisma/schema.prisma` | `fivenines_auth` (`AUTH_DATABASE_URL`) |
| `@apps/nestjs` | `apps/nestjs/prisma/schema.prisma` | `fivenines` (`DATABASE_URL`) |

For **Local MCP** / CLI, set `projectCWD` to the app directory (or repo root if your command runs from there with `-schema` path). Hooks run on `prisma/schema.prisma` under the edited tree.

1. Authenticate **Prisma-Remote** only if you use Prisma Cloud.
2. Prefer **Local MCP** (`migrate-dev`, `migrate-status`, `Prisma-Studio`) for day-to-day dev.
3. See app `AGENTS.md` for `db:migrate`, `db:seed`, compose profiles (`auth`, `nestjs`).
...
No implementation steps are required for this overview. If you want Prisma added to fivenines next, say which app/package and database (likely Postgres from Compose).
