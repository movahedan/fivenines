...
Player UI: `bun run container up -- --profile web` then open `http://play.fivenines.test:3001` after adding the `/etc/hosts` line in [docs/CHEATSHEET.md](./docs/CHEATSHEET.md) (starts Postgres + Nest too; login needs the auth profile or `all`).

Control-plane API only: `bun run container up -- --profile nestjs` then open `http://localhost:3006/api/docs`.

Auth service: `bun run container up -- --profile auth` — `http://auth.fivenines.test:3007/login`. For auth + Nest together, see [docs/CHEATSHEET.md](./docs/CHEATSHEET.md#auth--nestjs-smoke).
