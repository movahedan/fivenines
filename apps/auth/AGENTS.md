...
Login/register/OTP **302** to the allowlisted `redirect_uri` (or relative `next`) and set HttpOnly `auth_access` plus public `was_logged_in` on `AUTH_COOKIE_DOMAIN` (default `.fivenines.com`). Login/register/OTP page links keep `redirect_uri` (do not drop the query). `GET /logout` clears cookies, then **302** to the same allowlisted `redirect_uri` (or relative `next`); with no return params it renders the signed-out HTML page. Local HTTP: `AUTH_COOKIE_SECURE=false`. Names: `play.fivenines.com`, `auth.fivenines.com`, `api.fivenines.com` (see [CHEATSHEET](../../docs/CHEATSHEET.md)).
...
- Human JWT `aud` = `AUTH_AUDIENCE` (`fivenines-api`); Nest verifies via `AUTH_JWKS_URL` (see `apps/nestjs/AGENTS.md`).
- Seed demo tenant: `00000000-0000-4000-8000-000000000001` (slug `demo`) — same id as `@apps/nestjs` seed.
- M2M client `nestjs-control-plane` (`feature-flags:read`); machine JWT `aud` = `AUTH_AUDIENCE_EVAL` (`fivenines-eval`).
