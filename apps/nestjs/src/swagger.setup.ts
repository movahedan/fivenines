...
export async function swaggerSetup(app: INestApplication): Promise<OpenAPIObject> {
	...
	SwaggerModule.setup("api/docs", app, document, {
		customSiteTitle: "Fivenines Feature Flags Control Plane",
	});
...
function buildSwaggerConfig() {
	return new DocumentBuilder()
		.setTitle("Fivenines Feature Flags Control Plane")
		.setDescription(
...
async function emitOpenApi(document: OpenAPIObject): Promise<void> {
	...
	await runSdkGenerate();
}

async function runSdkGenerate(): Promise<void> {
	return new Promise((resolvePromise, reject) => {
		const child = spawn(process.execPath, ["run", "generate"], {
			cwd: NESTJS_SDK_PATH,
			stdio: "inherit",
		});
		child.on("close", (code) =>
			code === 0 ? resolvePromise() : reject(new Error(`sdk generate exited ${code}`)),
		);
