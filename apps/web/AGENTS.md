...
- `@packages/analytics`: `initAnalytics` + Silktide in `src/routes/-bootstrap-web-client.ts` (after first paint). GTM only when production, `VITE_GTM_CONTAINER_ID` set, and analytics consent. PWA via `vite-plugin-pwa` `generateSW` (`start_url: /`, Play shortcut `/hub`). No Firebase / FCM. Silktide look is `public/silktide/fivenines-consent.css` (ops green on `#050912`), loaded after the vendor CSS in `__root.tsx`.
...
| File | Route |
|------|--------|
| `src/routes/__root.tsx` | Root document shell (`html` / `head` / `#root` / `Outlet`). No `AuthProvider`, fetcher, or `QueryClient`. Silktide vendor CSS + `fivenines-consent.css` + Consent Mode script tags. |
...
`src/hub/` is the player ops console: `GameTemplate` shell (status bar, Projects rail, center floor, Inventory/Learning/Finances destinations, Activity overlay) composed from `@packages/ui`. Fleet/market `ServerCard` `dotClassName` comes from `SKU_DOT_CLASS` in `hub-map.ts` (token utilities, not hex). SKU CPU is `formatters.cores(computeUnitsPerHour)` (e.g. `1000 cores`); offer **DEMAND** is baseline RPS (`120 RPS`), not cores pretending to be capacity. Fleet bars are CPU/NET/RAM/DISK from `server.metrics`; GPU on teaching SKUs is **unavailable**, not a fake 0% bar. Market comparison also lists disk MiB and GPU `none`. Status-bar clock uses `GameClockControls` (pause is speed 0) and SLA percents use `@packages/shared/formatters` (`DAY 01 · HR 00:00`, `99.00%`). `use-hub-game.ts` owns `Game(openingInitial)`, interval `tick()` (skipped while paused **or** when `hourIndex >= OPENING_SHIFT_HOURS`), and `dispatch`. HUD time is `game.hourIndex`, not SSE `at`. Account chrome is `useAuth()` in web (sign-out slot). Sign out sends the browser to `@apps/auth` `logoutHref({ redirectUri: "/" })`; auth 302s to that allowlisted URI after clearing cookies. Visiting `/hub` without a session uses `loginHref({ redirectUri: "/hub" })`. Molecules stay engine-agnostic: the offer and active cards gained optional `serverOptions` / `selectedServerId` / `onSelectServer` / `onRoute` / `onUnassign` props that take plain ids, labels, and callbacks. Incoming ACCEPT no longer passes a fleet picker; optional `serverOptions` remain for later Start/MOVE.

Decline dispatches `declineProject` (offer leaves Incoming; allowed while jailed). ACCEPT opens **Contract Review** (`aria-label="Contract Review"`): Back keeps the selected offer, Close never charges, Accept contract dispatches `acceptProject` `{ projectId }` (no `serverId`). Accept is disabled while jailed; empty fleet is allowed. Advance posts on Accept contract only. Setup rows sit in Active: Assign box (`placeSetup`) then Install Application Runtime / Install Relational Database / Configure connection (`installService` / `configureConnection`); Power on/off the assigned box. Completing them marks ready without starting. Start stays disabled until ready and a box exists. Cancel setup dispatches `cancelSetup`, Park is hidden. Add server stays on the market after the first buy. Served cards get **MOVE** and **PARK** plus **Duplicate this project** (`duplicateProject` to the picker destination; pending copy stays unready). parked cards get **ASSIGN** and render in the Active grid after the served rows, with a `Parked (n)` counter in the Active panel header’s trailing slot. `No served projects` shows only when accepted, served, and parked lists are empty. Active cards show current-hour vs rolling 168h vs target vs recovery ETA (`slaRecoveryHours`); status/tone still use window vs `targetPpm`. Served/parked rows also show engine service state, this-hour handled/emitted, last settlement credit, and Telemetry **unavailable** (no reconstructed traces). The Active header includes `game.pathHour` (`ok` / `miss` / `pending`). The card’s server line is the routed box and its tier (`server-1 · Bronze`), or `PARKED`. Sparkline bars color vs `targetPpm/1e6` (empty → warming). HUD: **CASH**, **Receivable today** (`accountsReceivableCents`), **OPEX / hour** (includes lease rent), **LEARN** (`slotsUsed` / 2), **OPS** (`operations.slotsUsed` / 1, separate from LEARN). Active green digit is **WTD revenue** (`periodPaygCents`). Event log: Hub command lines plus `game.events` after each tick (SLA, PAYG settle, weekly credit, saturation, cash low); `commandLogTone` in `hub-map.ts` is an exhaustive `Record<EngineCommand["type"], EventLogTone>` (`startProject`/`leaseServer`/`enqueueOperationalTask` success, `acceptProject` info, `cancelSetup`/`releaseServer`/`cancelOperationalTask` warn, `enrollLearning`/`resumeLearning` success, `pauseLearning`/`cancelLearning` warn). Learning strip lists catalog rows with authored durations/prices (Enroll / Pause / Resume). Copy states that skill effects are not applied to missing consumers and that issue **#66** shared-asset identity is incomplete. Fleet rack cards reuse `ServerCard` in acquisition and Inventory; Inspect opens a bottom object inspector. Parked cards use **Resume**. Workspace nav is Projects / Floor / Business. Root layout is `h-screen overflow-hidden`; the event log is a bounded `h-40` scroller (`overflow: scroll`) so new lines stay reachable. At hour 336 the tick loop stops and a result overlay uses `openingShiftOutcome` (win: positive cash, ≥2 contracts meeting window target, no 100% credit, not jailed). **Reset** builds `Game(openingInitial)` and clears the log.
...
Demand inspect (`src/lab/demand-inspect.ts`) samples Appointment Site via DemandEngine for the current lab hour. Roots are not placed and `WorkQueue` is not executed. Path graphs stay a kernel `compileDemandGraph` lookup; Lab does not rebuild them.
