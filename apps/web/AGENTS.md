...
Decline dispatches `declineProject` (offer leaves Incoming; allowed while jailed). ACCEPT opens **Contract Review** (`aria-label="Contract Review"`): Back keeps the selected offer, Close never charges, Accept contract dispatches `acceptProject` `{ projectId }` (no `serverId`). Accept is disabled while jailed; empty fleet is allowed. Advance posts on Accept contract only. Setup rows sit in Active: Start stays disabled until ready (not this slice), Cancel setup dispatches `cancelSetup`, Park is hidden. Served cards get **MOVE** and **PARK**; parked cards get **ASSIGN** and render in the Active grid after the served rows, with a `Parked (n)` counter in the Active panel header’s trailing slot. `No served projects` shows only when accepted, served, and parked lists are empty. Active cards show current-hour vs rolling 168h vs target vs recovery ETA (`slaRecoveryHours`); status/tone still use window vs `targetPpm`. The card’s server line is the routed box and its tier (`server-1 · Bronze`), or `PARKED`. Sparkline bars color vs `targetPpm/1e6` (empty → warming). HUD: **CASH**, **Receivable today** (`accountsReceivableCents`), **OPEX / hour** (includes lease rent), **LEARN** (`slotsUsed` / 2). Active green digit is **WTD revenue** (`periodPaygCents`). Event log: Hub command lines plus `game.events` after each tick (SLA, PAYG settle, weekly credit, saturation, cash low); `commandLogTone` in `hub-map.ts` is an exhaustive `Record<EngineCommand["type"], EventLogTone>` (`startProject`/`leaseServer` success, `acceptProject` info, `cancelSetup`/`releaseServer` warn, `enrollLearning`/`resumeLearning` success, `pauseLearning`/`cancelLearning` warn). Learning strip lists catalog rows with authored durations/prices (Enroll / Pause / Resume). Copy states that skill effects are not applied to missing consumers and that Projects/Inventory shared-asset identity (#66) is still incomplete. Root layout is `h-screen overflow-hidden`; the event log is a bounded `h-40` scroller (`overflow: scroll`) so new lines stay reachable. At hour 336 the tick loop stops and a result overlay uses `openingShiftOutcome` (win: positive cash, ≥2 contracts meeting window target, no 100% credit, not jailed). **Reset** builds `Game(openingInitial)` and clears the log.
