...
- **Paths** — `EntityPackage`: `root` → repo root; `@packages/foo` → `packages/foo`; `@tools/foo` → `tools/foo`; `@apps/nestjs` → `apps/nestjs` (name must match `package.json` `name`).
...
After a successful prepare write, the script may append `packages-to-deploy=<name>` to `GITHUB_OUTPUT` when `docker-compose.yml` defines a **service** whose name equals the versioned package name (e.g. package `@apps/nestjs` → compose service `nestjs`). Downstream workflows can consume that output.
