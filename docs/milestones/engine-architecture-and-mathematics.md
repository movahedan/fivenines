...
**Current `Game.tick` order** ([#113](https://github.com/movahedan/fivenines/issues/113)): named playlist `#simulateHour` → `hourIndex += 1` → `#advanceCalendar`. Simulate: reset demand → `project.tick` / place demand → `server.tick` → SLA → opex → learning → PAYG accrue → sticky jail. Calendar: daily PAYG settle → `project.tickCalendar` → `spawnAcquaintanceIfDue` → week close. Entities expose their hour; Game sequences, runs shared-capacity steps, and commits `postCashDelta`. No plugin `TickPhase[]` registry, no second graph, no managers that copy `customers[]`. `dispatch` is immediate and does not advance the clock. See [engine AGENTS](../../packages/fivenines-engine/AGENTS.md). Milestone 5 replaces the place-demand step with allocation inside that playlist, not a new clock.
