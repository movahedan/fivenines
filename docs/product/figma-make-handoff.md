...
Apply the [infrastructure editor revision](infrastructure-editor.md) when extending this historical handoff: horizontal Requirements, full-width software rows, persistent automatic preparation checklist, valid-only target overlay, visible manual host connection anchors, automatic-only layout and compact non-editor server views. Preserve the original exported source; implement production shared components separately. The handoff below is read subject to these explicit amendments, not as authority to restore superseded manual setup or universal rack reuse.

## Prompt to use with the attached product documents

Design Five Nines, a game about running an infrastructure operations business for customers. Create a coherent, interactive desktop and mobile prototype using the attached product reference. Preserve its existing dark navy and restrained green console identity, with readable human-facing text and monospaced metrics. Use English throughout. The infrastructure system is the main play surface, with rack-shaped server containers and the current project's installed software inside them. These shapes do not introduce physical rack-space mechanics. Use the main project workspace rack for the editor, with the approved full-width rows; acquisition and Inventory use compact context-appropriate views.
...
Use mobile bottom navigation ordered Projects, Inventory, a prominent central New project action, Learning, Finances. On desktop, use expandable/collapsible folder-like rail panels with vertically labeled tabs. Keep an important-status top bar on both. On desktop, Projects occupies the left panel and Inventory/Learning/business Finances share the right panel. Distinct Operations and two-slot Learning progress sit in the top bar; mobile uses a progress strip above navigation. Keep Activity reachable without covering the canvas in permanent alert cards.

New project opens the central offer page on desktop and the active content page on mobile. Browse existing offers with previous/next controls and a count; open full Contract Review as a separate step. Back preserves the selected offer and reading position. Only explicit acceptance collects the advance and opens setup. Acquisition and material-impact reviews retain bottom drawers with matching directional/scrolling semantics and visible back/close controls.

Inside a project, use a compact identity/action header and vertically arranged Contract, Infrastructure, and Setup checklist or Project status sections. Contract starts collapsed; Project status contains Status, Performance and Finances tabs below Infrastructure on both devices. Selected-object details use bottom drawers on both devices without taking over business navigation. Checklist actions, recovery and destination selection use the same contextual flow. Connect uses source selection, a Connect action and compatible destination selection, with no required dragging. Contain mobile pan/zoom inside Infrastructure and keep Add server available after acquisition. Use the interaction specification's object ownership, state-dependent primary actions, configuration/operations/removal groups, blocked-action reasons and impact reviews. Server actions can affect several projects. Service configuration is shared across that service's instances; health and placement are instance-specific. Never duplicate unrelated projects sharing a server.
...
| Acquisition flow | Offer browsing page, full details, contract review, acceptance, empty/expired/unaffordable states |
| Project workspace | Setup requirements, ready activation, healthy/degraded system, Park/Resume, recovery, ended contract |
