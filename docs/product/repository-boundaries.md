...
`packages/ui/src/templates/game-template` owns named section slots and their desktop-panel/mobile-navigation presentation. `GameStatusBar` is shared ops chrome with `static` and `absolute` (bottom of parent) placements. The [game shell map](game-shell.md) records Figma section ownership, 5.1 data versus visual-only states, and which package stores navigation/panel state. Parent (web, or Storybook) owns destination, panel open/width, and overlay open flags; the template owns rails, tabs, overlay chrome, and resize handles via `react-resizable-panels`. Overlay bodies are slots. Shared organisms include `pick-software` and `pick-infrastructure`; the temporary target-selection overlay is a molecule. Shared components take display data and callbacks and remain native-compatible, without engine/auth/React Flow imports. `apps/web/src/infrastructure-editor` owns the React Flow adapter, component-first `node-*` folders and automatic layout. `node-server` contains `node-server-software-row`; the rack is not a universal UI organism. Project workspace owns unapplied changes across all sections. The template path is implemented as a Storybook shell; Hub cutover and the editor tree remain later 5.1 slices. Engine eligibility/planning remains domain-owned.
...
| [@apps/figma-design](../../apps/figma-design/AGENTS.md) | Preserved Figma visual reference and local preview | Generated fixture source is isolated from production; no engine/auth dependency or deployment. Implement approved behavior in the real UI using the interface brief and interaction specification. |
