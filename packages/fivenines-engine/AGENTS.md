...
Catalog: Bronze–Diamond plus teaching `thin-ram` (`SERVER_CATALOG`). Bronze = compute **1000**, net **1_000_000**, memory **4096**, base **256**, disk **64 GiB** / **2000×3600** ops, **no GPU**. `oneBronzeInitial` routes both **constant** 700 RPS projects to `server-1` (exact **1400** against a 1000 cap, CPU-bound on one Bronze, fair 500/500 shares). `twoBronzeInitial` routes one project per box and is the **isolation** fixture: overload on one box never spills onto the other. `openingInitial`: Maya / Maya's Appointments only (`appointment-site` commercial, weekly fee 80 design units → 8000¢ via `DESIGN_UNIT_CENTS`, SLA 80% → 800_000 ppm, PAYG 0, baseline 120, `offerTtlHours: 48`), `assets: []`. Opening Shift still requires two **served** contracts to win. Completing the first-project setup checklist marks `ready`; it does not call `startProject`. Seeded first-week play through DemandEngine allocation, week close, park-overload credits, lease vs buy, and salvage recovery lives in `src/playtest.first-project.test.ts`. After the first offer leaves pending (accept, decline, expire, or cancel setup), another acquaintance-style appointment may spawn after 24h when pending cap is 1. Served acquaintance demand (shaped saas baseline 120) uses `DemandEngine.hourly("appointment-site")` inside `allocateHour`; other live RPS still uses `Project.tick` converted to CPU/net/disk/RAM work items.
...
Opening Shift lasts `OPENING_SHIFT_HOURS` (`BILLING_PERIOD_HOURS` = 168) in `src/catalog/opening-shift-policy.ts`. `openingShiftOutcome(snapshot)` is **in_progress** until `hourIndex >= 168`. After that it is **won** only if cash `> 0`, `jailed` is false, at least two **served** projects have `windowAvailabilityPpm >= targetPpm`, and no settlement landed in the catastrophe credit band (`slaCreditPpm(settlement.periodPpm, targetPpm) === SLA_CREDIT_CATASTROPHE_PPM`). Otherwise **lost** (failed reasons: `jailed` / `cash` / `contracts` / `catastrophe`). The one-week gate keeps a healthy two-contract owned Bronze cash-positive; a second unpaid opex week is what used to jail correct play. Pure helper — do not tick a full shift in the policy unit tests (the two-contract win lives in `src/playtest.first-project.test.ts`).
...
`src/demand-engine/` generates typed root batches. `Game` constructs `DemandEngine` for served/parked acquaintance (shaped saas baseline 120) inside `#placeDemand`. Constant teaching fixtures still emit via `Project.tick`. DemandEngine still does not run `Server.tick`.
...
- M5 (stacks on #111): [resource allocation and system execution](../../.cursor/plans/m5-resource-allocation-and-execution.plan.md) — base [#119](https://github.com/movahedan/fivenines/pull/119), allocator [#120](https://github.com/movahedan/fivenines/pull/120), paths [#121](https://github.com/movahedan/fivenines/pull/121), transfers [#122](https://github.com/movahedan/fivenines/pull/122). Playtest is a later slice.
