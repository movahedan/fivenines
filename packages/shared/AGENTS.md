...
Cross-app helpers that are not UI-specific: logging, cookies, integer units, id uniqueness, and display formatters (cents, `TICK`/`DAY · HR` clock, calendar `simDate`, ppm as percent, cores). No product/engine types.
...
`formatters` is display-only (`cents`, `hourTick` → `TICK 0000`, `clockLabel` → `DAY 01 · HR 00:00` with 1-based day, `simDate` → `Sep 9, 2026` from origin 2026-09-09 UTC plus whole sim days, `ppm` → `99.00%` from integer ppm/10_000, `cores` / `coresCompact`). Kernel values stay integers; these functions only shape strings. It must not import engine, auth, or UI.
