...
**Ops chrome** (engine-agnostic display props + callbacks; no `@packages/fivenines-engine` / `@packages/auth`): `Hud` (default **9s** mark + `shadow-glow-primary`; optional `logo` slot; lucide Pause/Play icon with accessible names Pause/Play; `speed` `1|2|4` icon buttons ×1/×2/×4), `PanelHeader` (tone dot + matching `shadow-glow-*`), `MetricStat`, `ProjectOfferCard` (`disabled` gates Accept only; Decline stays enabled), `ActiveProjectCard` (`slaPercent` 0–100, SLA caption rows, `sparkline` 0–1 vs `sparklineTarget`), `ServerCard` (`variant`: `fleet` | `market`; optional `dotClassName` token utilities for the SKU glow dot), `EventLog`. Pass region/SKU color via token `className` (`text-info`, `bg-warning shadow-glow-warning`, …), not hex. Barrel: `@packages/ui/molecules`.
