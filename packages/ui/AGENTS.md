...
**Ops chrome** (engine-agnostic display props + callbacks; no `@packages/fivenines-engine` / `@packages/auth`): `Hud` (default **9s** mark + `shadow-glow-primary`; optional `logo` slot; lucide Pause/Play icon with accessible names Pause/Play; `speed` `1|2|4` icon buttons ×1/×2/×4), `GameStatusBar` (`placement` `static` in flow or `absolute` stuck to the bottom of a relative parent), `GameClockControls` (sim date, day-progress bar, pause/0/1/2/4× — not a calendar picker), `GameAccountControl` (OP badge + operator identity), `GamePipelineChip` (ops/learn progress chip or flush mobile row), `GameLearningPanel` (Technologies/Courses catalog, ongoing banner, inline enroll detail), `PanelHeader` (tone dot + matching `shadow-glow-*`), `MetricStat`, `ProjectOfferCard` (`disabled` gates Accept only; Decline stays enabled; demand metric is a string, not cores), `ActiveProjectCard` (`slaPercent` 0–100, SLA caption rows, sparkline 0–1 vs `sparklineTarget`, optional service/work/credit/telemetry strings), `ServerCard` (`variant`: `fleet` | `market`; optional disk/GPU stats; fleet GPU may be an unavailable label instead of a bar; optional `dotClassName` token utilities for the SKU glow dot), `EventLog`. Pass region/SKU color via token `className` (`text-info`, `bg-warning shadow-glow-warning`, …), not hex. Barrel: `@packages/ui/molecules`. `GameTemplate` (`@packages/ui/templates`) is a controlled desktop/mobile shell: parent owns destination, panel open/width, and overlay open state; the template owns 28px folder tabs, overlay chrome, bottom-drawer overlay, and `react-resizable-panels` grips (unpainted). Overlay/drawer bodies are slots. Hub composes `GameTemplate`.
...
Canonical source: `src/style.css` (`:root` and `.dark` share the **ops** navy/neon palette). `@theme inline` maps CSS vars to NativeWind/Tailwind utilities (`bg-background`, `text-primary`, `bg-hud`, `bg-overlay-scrim`, `shadow-glow-primary`, …). `@source` includes `atoms`, `shadcn`, `molecules`, and `templates`. Overlay/drawer layers use inline `position` and `bg-overlay-scrim` (RN-web Pressable/View inline `backgroundColor` is forced transparent). Glow shadows: `shadow-glow-primary`, `shadow-glow-danger`, `shadow-glow-warning`, `shadow-glow-info`, `shadow-glow-sla` (RGB lockstep with OPS hex; mirrored on `theme.ts` `OPS.glow*`). Fonts: Inter (`font-sans`) and JetBrains Mono (`font-mono`) via Google Fonts `@import` in `style.css`. `src/theme.ts` `THEME.light` / `THEME.dark` and `NAV_THEME` must stay in lockstep with those hex values — do not keep a second light palette.
