...
async function applyMissingKeysFromEnvFile(filePath: string): Promise<void> {
	...
	for (const fileLine of text.split("\n")) {
		const parsed = parseEnvFileLine(fileLine);
		if (parsed === undefined) {
			continue;
		}
...
export function expandComposeSubstitution(input: string, env: NodeJS.ProcessEnv): string {
...
export async function resolveServiceHostPorts(
	...
	await loadComposeEnvDefaults(repoRoot);
